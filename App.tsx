...
import { generateFeedbackFromGemini } from './services/geminiService';
import { SparklesIcon } from './components/Icons'; // Assuming Icons.tsx for these
...
const App: React.FC = () => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setFeedback(null);
    try {
      const result = await generateFeedbackFromGemini(inputText);
      setFeedback(result);
    } catch (err) {
      if (err instanceof Error) {
        setError(`Erro ao gerar feedback: ${err.message}. Verifique se a API Key está configurada corretamente.`);
      } else {
        setError('Ocorreu um erro desconhecido ao gerar feedback.');
      }
      console.error(err);
    } finally {
      setIsLoading(false);
