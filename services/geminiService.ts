...
const ai = new GoogleGenAI({ apiKey: import.meta.env.VITE_GEMINI_API_KEY || "MISSING_API_KEY" }); // Provide a dummy if not set to avoid immediate crash, SDK handles it.

export const generateFeedbackFromGemini = async (userText: string): Promise<string> => {
  if (!import.meta.env.VITE_GEMINI_API_KEY || import.meta.env.VITE_GEMINI_API_KEY === "MISSING_API_KEY") {
